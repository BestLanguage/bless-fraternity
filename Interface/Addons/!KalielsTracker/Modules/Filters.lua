...
local function Filter_Quests(spec, idx)
	...
	elseif spec == "zone" then
		...
		for i=1, numEntries do
			...
			if isHeader then
				headerName = title
				isInZone = (strfind(zoneName, title) or (subzoneName ~= "" and strfind(subzoneName, title)) or KT.playerClass == title)
			else
				if not isInZone then
					local _, questDescription = GetQuestLogQuestText(i)
					isInZone = (strfind(questDescription, zoneName) or
								(subzoneName ~= "" and strfind(questDescription, subzoneName)))
				end
